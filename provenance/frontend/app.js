...
function clearResults() {
  ...
  timeline.innerHTML = "";
  matches.innerHTML = "";
  report.innerHTML = "<h3>Investigation evidence</h3><p>Analysis findings will appear here.</p>";
}
...
steps.forEach((step) => {
  step.addEventListener("click", () => {
    ...
    document.querySelector(`#${step.dataset.view === "upload" ? "dropzone" : step.dataset.view}`).scrollIntoView({ behavior: "smooth", block: "start" });
  });
});
...
button.addEventListener("click", async () => {
  ...
  report.innerHTML = "<h3>Investigation evidence</h3><p>Running forensic analysis...</p>";
  let response;
  let body;
  try {
    ...
    if (error.name === "AbortError" || requestGeneration !== analysisGeneration) return;
  ...
  steps.forEach((step) => step.classList.toggle("active", step.dataset.view === "report"));
  renderResults(body);
  analysisController = undefined;
  input.value = "";
});
...
reset.addEventListener("click", () => {
  ...
  steps.forEach((step) => step.classList.toggle("active", step.dataset.view === "upload"));
  preview.innerHTML = "<h3>Evidence preview</h3><p>Select a file to preview it here.</p>";
});

function renderResults(data) {
  const ai = data.ai_assessment || data.video_analysis?.ai_assessment;
  const aiConfidence = ai?.confidence ?? ai?.artificial_score ?? 0;
  const faces = data.face_analysis?.face_count ?? data.video_analysis?.face_summary?.max_faces ?? 0;
  const semantic = data.image_embedding?.vector ? "Available" : "Unavailable";
  summary.classList.remove("hidden");
  story.classList.remove("hidden");
  summary.innerHTML = [
    ["Media", data.media_type.toUpperCase(), "type"],
    ["AI assessment", ai?.label || "unknown", `${Math.round(aiConfidence * 100)}% model score`],
    ["Faces", faces, "detected"],
    ["Semantic DNA", semantic, "CLIP embedding"],
  ].map(([title, value, note]) => `<article class="metric"><small>${title}</small><strong>${value}</strong><span>${note}</span></article>`).join("");
  const sections = data.media_type === "video"
    ? { "Video analysis": data.video_analysis, "AI evidence": { model_score: `${Math.round(aiConfidence * 100)}%`, frame_consensus: ai?.ai_frame_consensus === undefined ? "Unavailable" : `${Math.round(ai.ai_frame_consensus * 100)}%` }, "Propagation trace": summarizeTrace(data.platform_trace), "Metadata": { codec: data.codec, duration_seconds: data.duration_seconds, dimensions: `${data.width} × ${data.height}` }, "Media DNA": { sha256: data.sha256, embedding: semantic } }
    : { "AI assessment": data.ai_assessment, "Face analysis": data.face_analysis, "Forgery analysis": data.forgery_analysis, "Metadata": data.metadata, "Transformation": data.transformation_analysis, "Propagation trace": summarizeTrace(data.platform_trace), "Media DNA": { sha256: data.sha256, perceptual_hash: data.perceptual_hash, embedding: semantic } };
  report.innerHTML = `<h3>Investigation evidence</h3><div class="findings">${Object.entries(sections).map(([title, value]) => `<details open><summary>${title}</summary>${renderValue(value)}</details>`).join("")}</div>`;
  renderStory(data);
}
...
function renderStory(data) {
  ...
  const allPosts = platforms.flatMap((platform) => (platform.posts || []).map((post) => ({ ...post, platform: platform.platform })));
  ...
  const observedPlatforms = (data.platform_trace?.platforms || []).filter((platform) => platform.status === "available").length;
  const exactMatches = (data.platform_trace?.platforms || []).flatMap((platform) => platform.posts).filter((post) => post.exact_hash_match).length;
  const matchSignals = [
    ["SHA-256 identity", data.sha256 ? "Available" : "Unavailable"],
    ["Perceptual fingerprints", data.perceptual_hash ? "Available" : "Unavailable"],
    ["Semantic embedding", data.image_embedding?.vector?.length ? `${data.image_embedding.vector.length} dimensions` : "Unavailable"],
    ["Platforms checked", `${observedPlatforms}/3`],
    ["Exact platform matches", String(exactMatches)],
  ];
  matches.innerHTML = `<p class="eyebrow">MATCHES & SOURCES</p><h2>Propagation trace</h2><div class="match-list">${matchSignals.map(([label, value]) => `<div class="match-row"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join("")}</div><p class="panel-note">The trace checks the simulated InstaMock, XMock and FaceMock feeds for media attached to posts.</p>`;
  matches.innerHTML += `<div class="lineage"><p class="eyebrow">MEDIA LINEAGE</p><div class="lineage-grid"><article class="lineage-card original"><span class="lineage-kind">ORIGINAL UPLOAD</span><strong>${escapeHtml(data.uploaded_filename || "Uploaded evidence")}</strong><p>Analyzed reference media</p></article>${relevantPosts.slice(0, 6).map((post) => renderMatchedMedia(post, data.media_type)).join("") || `<p class="empty-value">No matching platform media found.</p>`}</div></div>`;
...
function renderMatchedMedia(post, mediaType) {
  ...
  const kind = post.exact_hash_match ? "EXACT COPY" : "CANDIDATE TRANSFORMED COPY";
...
function summarizeTrace(trace) {
  ...
  return {
    ...
    exact_matches: platforms.reduce((total, platform) =>
      total + (platform.posts || []).filter((post) => post.exact_hash_match).length, 0),
    matched_platforms: platforms
      .filter((platform) => (platform.posts || []).some((post) => post.exact_hash_match))
...
function renderLineageGraph(lineage) {
  ...
  const nodes = [
    { label: "Origin", detail: lineage.origin?.file || "Original media", kind: "origin" },
...
function renderValue(value) {
  ...
  return `<div class="facts">${Object.entries(value).map(([key, item]) =>
    `<div class="fact"><span>${formatLabel(key)}</span><div class="fact-value">${renderValue(item)}</div></div>`
  ).join("")}</div>`;
}

function formatLabel(key) {
  return escapeHtml(key.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase()));
}
